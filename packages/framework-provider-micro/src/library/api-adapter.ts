...
export type APIResult = { status: 'success'; result: unknown } | { status: 'failure'; code: number; title: string; reason: string }

export async function requestSucceeded(body?: unknown): Promise<APIResult> {
    return {
        status: 'success',
        result: body,
    }
